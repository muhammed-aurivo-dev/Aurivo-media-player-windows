...
getId("back").addEventListener("click", () => {
	ipcRenderer.send("load-win", join(__dirname, "..", "html", "index.html"));
});
...
// NOTE: Language selection is handled by the main Aurivo app (single source of truth).
// Dawlod window will follow the app language via injected locale sync.
// Keep any leftover logic guarded to avoid errors if upstream HTML changes.
try {
	const selectEl = getId("select");
	if (selectEl) {
		const language = localStorage.getItem("locale");
		if (language) {
			selectEl.value = language.startsWith("en") ? "en" : language;
		}
		// Kept for compatibility if the selector is reintroduced.
		globalThis.changeLanguage = function changeLanguage() {
			const next = selectEl.value;
			localStorage.setItem("locale", next);
			localStorage.setItem("localeUserSelected", "true");
			ipcRenderer.send("set-locale", next);
		};
	}
} catch {}
